...
@WebAppConfiguration
class OshTeacherControllerTest {
    @Autowired
    ...
    void search() throws Exception {
        MvcResult result = mvc
                .perform(get("/osh/worker/search?field=name")
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andReturn();
        assertEquals(200, result.getResponse().getStatus());
    ...
    void updateTeacher() throws Exception{
        TeacherDTO teacher = new TeacherDTO("name", "surname", null,
                "aaaa@gmail.com", "+996 (500) 12 12 32", 6, 4,
                "1234567890",   LocalDate.now(), LocalDate.now(), "desc");
        String jsonRequest = mapper.writeValueAsString(teacher);
        MvcResult result = mvc
                .perform(put("/osh/worker/{id}", 2)
                        .content(jsonRequest)
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andReturn();
    ...
    void deleteTeacher() throws Exception {
        MvcResult result = mvc
                .perform(delete("/osh/worker/{id}", 1)
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andReturn();
        assertEquals(200, result.getResponse().getStatus());
